...
根据岗位信息填写职位名称、职位描述、学历、日薪、实习月数、每周到岗天数、账号已有地址和电话交换助手，并逐字段回读核验。

#### 社招全职

根据岗位信息填写职位名称、职位描述、经验、学历、月薪、账号已有地址和电话交换助手，并逐字段回读核验。

#### 应届校园招聘

根据岗位信息填写职位名称、职位描述、固定“在校/应届”经验、学历、月薪、毕业时间、账号已有地址和电话交换助手，并逐字段回读核验。

#### 兼职招聘

根据岗位信息填写职位名称、职位描述、经验、学历、结算方式、薪资单位及区间、兼职周期、工作时段、每周天数、班次、招聘截止日期、账号已有地址和电话交换助手，并逐字段回读核验。

四类岗位都通过 `prepare_job_post()` / `prepare` 填写并核验。任务要求完成发布且全部字段回读一致时，通过 `publish_reviewed_job()` / `publish-reviewed` 单次提交；若提交结果不确定，只调用 `reconcile_job_post()` / `reconcile` 只读核对，不重复发布。`get_run_status()` / `status` 可用于查看本次填写、核验、警告和提交状态。
